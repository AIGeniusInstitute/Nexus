...
async fn turn_start(AuthUser(c): AuthUser, State(st): State<AppState>, Path(id): Path<Uuid>, Json(req): Json<TurnReq>) -> Result<Json<Value>, (StatusCode, String)> {
    // Verify thread belongs to user's tenant.
    ...
    let row: (i64,) = sqlx::query_as("INSERT INTO turns (thread_id, status, started_at) VALUES ($1, 'running', NOW()) RETURNING id")
        .bind(id).fetch_one(&st.pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let turn_id = row.0;
    // M1 skeleton: emit a single system item (real execution is M2 runtime pool).
    let content = req.input.unwrap_or_else(|| "(empty turn)".into());
    sqlx::query("INSERT INTO items (thread_id, turn_id, seq, item_type, content_ref) VALUES ($1, $2, 1, 'system', $3) ON CONFLICT DO NOTHING")
        .bind(id).bind(turn_id).bind(&content)
